# -*- coding: utf-8 -*-
"""Unconditional U-Net score model for grayscale 28×28 images."""
...
class UNetScoreModel(BaseScoreModel):
    """Time-dependent score network for 1-channel 28×28 images.

    Implements a U-Net with group normalisation and random Fourier feature
    time embeddings.  Class labels are accepted but silently ignored, so the
    model is a drop-in replacement for the conditional variant when no
    conditioning is needed.

    Parameters
    ----------
    marginal_prob_std : callable
        ``sigma_t(t)`` — returns the marginal standard deviation for time *t*.
    channels : list of int
        Feature-map channel widths at each U-Net resolution.
    embed_dim : int
