# -*- coding: utf-8 -*-
"""Unconditional U-Net score model for 3-channel 32×32 images (e.g. SVHN)."""
...
class _Dense(nn.Module):
    """Fully-connected layer that broadcasts to feature-map shape."""
...
class UNetScoreModelColor(BaseScoreModel):
    """Unconditional U-Net score model for 3-channel 32×32 images.

    Structurally identical to :class:`UNetScoreModel` but uses
    ``padding=1`` throughout so that spatial dimensions are preserved at
    32×32 input (vs. the no-padding variant designed for 28×28).

    Class labels are accepted but silently ignored, keeping the interface
    compatible with :class:`CondUNetScoreModel` and :class:`CFGWrapper`.

    This model is API-compatible with the ``ScoreNet`` defined in
    ``score_model_digits_color.py`` and loads its checkpoints directly.

    Parameters
    ----------
    marginal_prob_std : callable
        ``sigma_t(t)`` — marginal standard deviation of the forward process.
    channels : list of int
        Feature-map widths at each U-Net resolution.  Default: [32, 64, 128, 256].
    embed_dim : int
        Dimension of the time-embedding vector.  Default: 256.
    """
    ...
    def forward(
        ...
        x: torch.Tensor,
        t: torch.Tensor,
        class_label: Optional[torch.Tensor] = None,  # ignored
    ) -> torch.Tensor:
